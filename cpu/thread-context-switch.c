...
int main(int argc, char *argv[]) {
    ...
    pthread_create(&thread, NULL, context_switch, NULL);

    int random_buf[1] = {0};

    // measure the time of read and write of pipe
    int measure_rw[2];  // for measuring read/write cpu clocks,
                        // measure_rw[0] is for reading, measure_rw[1] is
                        // for writing
    ...
    printf(
        ...
        "of pipe: %lu cycles\n",
        srw2_sum / N);
    uint64_t context_switching_time = (srw2_sum / N - 2 * rw_sum / N + 48) / 2;
